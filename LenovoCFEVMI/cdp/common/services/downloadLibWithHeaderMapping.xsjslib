...
function getTableContent(rs) {
	...
	/*************************************
	 * Upgraded by Chris Gao
	 * 2015-09-23
	 * header mapping
	 ************************************/
	var mappingTableHeader = String($.request.parameters.get('mappingTableHeader'));
	var mappingTableHeaderModel = JSON.parse(mappingTableHeader);
	var columnHeaderName = "";
	for(i = 1; i <=columnCount; i++ ){
		
		columnHeaderName = meta.getColumnName(i);
		if(mappingTableHeaderModel[columnHeaderName] !== undefined)
		{
			columnHeaderName = mappingTableHeaderModel[columnHeaderName];
		}
		tableHeader.append(columnHeaderName);
	}
