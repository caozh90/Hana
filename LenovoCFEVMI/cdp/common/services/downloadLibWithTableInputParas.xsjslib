...
function getQuery(){
	...
	var tablewithinputpara = $.request.parameters.get("tablewithinputpara");
	...
	var query = 'select * from (select ' + column + ' from (select * from ' + tablewithinputpara + modelIn + filter + sort + limitOffset + '))';
