...
function trigger()
{
		...
		update_query = 'call "SZEBGVMI"."cdp.szebgvmi.procedures.pkg_ui.ui_urgent_mo_maintence::ui_urgent_mo_pull_run"()';
		try
		{
...
else
{
	try
	{
		...
		{
			query = 'select count(1) from "_SYS_BIC"."cdp.szebgvmi.models/AT_URGENT_MO_MAINTENCE" where STATUS=\'DRAFT\'';
			...
			if(status_count >0)
			{
				query = 'call "SZEBGVMI"."cdp.szebgvmi.procedures.pkg_ui.ui_urgent_mo_maintence::ui_urgent_mo_pull_all"()';
		...
		else if(job_type === "ready")
		{
			...
			if(status_count >0)
			{
				...
				query = ' call "SZEBGVMI"."cdp.szebgvmi.procedures.pkg_ui.ui_urgent_mo_maintence::ui_urgent_mo_pull_ready"(\'' +
				mo_id+'\')';
				pcall = conn.prepareCall(query);
				pcall.execute();
				pcall.close();
		...
		{
			query = 'select count(1) from "_SYS_BIC"."cdp.szebgvmi.models/AT_URGENT_MO_MAINTENCE" where STATUS=\'READY\'';
			pcall_status = conn.prepareStatement(query);
			rs_status = pcall_status.executeQuery();
			status_count=0;
			if(rs_status.next())
