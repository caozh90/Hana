...
function validationBefore(param,oper_type){
    ...
    pStmt = param.connection.prepareCall('call "cdp.szebgvmi.procedures.pkg_ui.ui_nocut_so_priority::validationNocutSoPriority"(?,?,?,?)');
...
function insert(param){
    ...
    pStmt = param.connection.prepareCall('call "cdp.szebgvmi.procedures.pkg_ui.ui_nocut_so_priority::createNocutSoPriority"(?,?)');
    pStmt.setNString(1,LOGICAL_PLANT);
	pStmt.setInteger(2,DELIVERY_PRIORITY);
    pStmt.execute();
}
...
function remove(param){
    ...
    pStmt = param.connection.prepareCall('call "cdp.szebgvmi.procedures.pkg_ui.ui_nocut_so_priority::deleteNocutSoPriority"(?,?)');
    pStmt.setNString(1,LOGICAL_PLANT);
	pStmt.setInteger(2,DELIVERY_PRIORITY);
    pStmt.execute();
}
