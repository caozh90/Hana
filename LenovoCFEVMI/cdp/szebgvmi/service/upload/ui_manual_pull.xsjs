...
var SRC_WERKS;
var PHYSICAL_PLANT = ""; //updated by Chris Gao 2015-09-22
var INVENTORY_TYPE;
var PRODUCTION_LINE = ""; //updated by Chris Gao 2015-09-22
var HEAD_SUPPLIERID;
...
function exception_process()
{
	...
	var info_sql = 'call "SZEBGVMI"."cdp.szebgvmi.procedures.pkg_ui.upload::ui_manual_pull_err"('
		+'?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
	...
	try
	{
		...
		pcall2.setString(11,DELIVERY_DATE);
...
if(lib.assertAppRole(role1)==='OK'||lib.assertAppRole(role2)==='OK'||lib.assertAppRole(role3)==='OK')
{
	...
	for(i=0;i<data.length;i++)
	{
		...
		if(data[i].PHYSICAL_PLANT !== undefined && data[i].PHYSICAL_PLANT !== null)
		{
		...
		if(data[i].PRODUCTION_LINE !== undefined && data[i].PRODUCTION_LINE !== null)
		{
		...
		if(data[i].DELIVERY_DATE !== undefined && data[i].DELIVERY_DATE !== null)
		{
			DELIVERY_DATE = processData(data[i].DELIVERY_DATE);
		}
		else
		{
			DELIVERY_DATE = (new Date()).toString("yyyy-MM-dd hh:mm:ss");
		}
	...
	/*	if(DEST_STORAGE_LOC === undefined ||DEST_STORAGE_TYPE === undefined ||DEST_STORAGE_BIN === undefined ||SRC_WERKS === undefined )
		...
		*/
		...
		query = 'CALL "SZEBGVMI"."cdp.szebgvmi.procedures.pkg_ui.upload::ui_manual_pull"('
				+'?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
		try
		{
	...
	query = 'call "SZEBGVMI"."cdp.szebgvmi.procedures.pkg_ui.upload::ui_manual_pull_delete"()';
