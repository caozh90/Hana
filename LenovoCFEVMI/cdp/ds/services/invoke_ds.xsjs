...
if(TABLE_NAME==='MXEBGVMI.LOI_GR' || TABLE_NAME === 'MXEBGVMI.SOI_GR')
{
	status = 'NEW';
}
if(TABLE_NAME==='MXEBGVMI.CLEAR_DIFF' || TABLE_NAME==='MXEBGVMI.PULL_LINE')
{
	status = 'COMPLETED';
}
if(TABLE_NAME ==='MXEBGVMI.UI_URGENT_MO_PULL')//目前，这个页面单独调用DS
{
	status = 'RUNNING';
}
...
function run_job()
{
	...
	var locked_sp = 'call "DS_INFO"."cdp.ds.procedures::job_locked"(?,?)';
...
function trigger()
{
		...
		for(i=0;i<P_KEY.length;i++)
		{
			...
			if(TABLE_NAME==='MXEBGVMI.PULL_LINE')
			{
				update_query = 'update '+TABLE_NAME+' set '+SEGMENT_NAME+
			...
			{
				//更改表中相应记录的状态
				update_status_sp = 'CALL "DS_INFO"."cdp.ds.procedures::update_status"(\''
					+ update_query
					+'\')';
				...
				//添加更改用户和更改时间信息
				modified_info_sp = 'CALL "DS_INFO"."cdp.ds.procedures::update_modified_info"(\''
					+ TABLE_NAME +'\',\''
					+ assignment
					+'\')';
