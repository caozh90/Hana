...
if (lib.assertAppRole(role) === 'OK') //(role === 'ui_cryad_ph::uploadable')//
{
	...
	if(success_result.length>0){
		try
		{
			var callProcedure = hconn.loadProcedure('EBGCFE', 'cdp.ebgcfe.procedures.pkg_ui.upload::ui_cryad_ph');
	...
	if(error_result.length>0){
		try
		{
			var callProcedure = hconn.loadProcedure('EBGCFE', 'cdp.ebgcfe.procedures.pkg_ui.upload::ui_cryad_ph_err');
