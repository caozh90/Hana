...
function exception_process()
{
	...
	var info_sql = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_mobom_location_err"(?,?,?,?,?,?,?,?)';
...
if(lib.assertAppRole(role)==='OK')
{
	...
	for(i=0;i<data.length;i++)
	{
		...
		query = 'CALL "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_mobom_location"(?,?,?,?,?,?,?)';
		try
		{
