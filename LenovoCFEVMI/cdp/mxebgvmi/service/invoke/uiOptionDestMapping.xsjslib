...
function validationBefore(param,oper_type){
    ...
    pStmt = param.connection.prepareCall('call "cdp.mxebgvmi.procedures.pkg_ui.ui_option_dest_mapping::validationOptionDestMapping"(?,?,?,?,?,?,?,?,?,?)');
...
function insert(param){
    ...
    pStmt = param.connection.prepareCall('call "cdp.mxebgvmi.procedures.pkg_ui.ui_option_dest_mapping::createOptionDestMapping"(?,?,?,?,?,?,?,?)');
    pStmt.setNString(1,LOGICAL_PLANT);
	pStmt.setNString(2,WAREHOUSE_OWNER);
	pStmt.setNString(3,SOURCE_LOCATION);
	pStmt.setNString(4,SOURCE_TYPE);
...
function update(param){
    ...
    pStmt = param.connection.prepareCall('call "cdp.mxebgvmi.procedures.pkg_ui.ui_option_dest_mapping::updateOptionDestMapping"(?,?,?,?,?,?,?)');
    pStmt.setNString(1,LOGICAL_PLANT);
	//pStmt.setNString(2,WAREHOUSE_OWNER);
...
function remove(param){
    ...
    pStmt = param.connection.prepareCall('call "cdp.mxebgvmi.procedures.pkg_ui.ui_option_dest_mapping::deleteOptionDestMapping"(?,?,?,?,?)');
	pStmt.setNString(1,LOGICAL_PLANT);
	pStmt.setNString(2,SOURCE_LOCATION);
	pStmt.setNString(3,SOURCE_TYPE);
	pStmt.setNString(4,SOURCE_BIN);
