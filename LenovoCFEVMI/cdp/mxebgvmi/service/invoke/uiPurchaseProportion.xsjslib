...
function validationBefore(param,oper_type){
    ...
    pStmt = param.connection.prepareCall('call "cdp.mxebgvmi.procedures.pkg_ui.ui_purchase_proportion::validationPurchaseProportion"(?,?,?,?,?,?,?,?,?,?)');
...
function remove(param){
    ...
    pStmt = param.connection.prepareCall('call "cdp.mxebgvmi.procedures.pkg_ui.ui_purchase_proportion::deletePurchaseProportion"(?,?,?,?)');
    pStmt.setNString(1,ITEM);
