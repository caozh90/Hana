...
var STATUS;
var MANUALPULLTYPE;//ZHAODAN1 20161107
var PRE_HEADER_ID;//ZHAODAN1 20161108
var LAST_PRE_HEADER_ID = "";//ZHAODAN1 20161116
...
function getPreHeaderId(manualPullType){
	pre_headerId = 'NO';
	query = 'SELECT "MXEBGVMI"."cdp.mxebgvmi.functions.ui_manual_pull::getPullHeaderLineID"(\'' + manualPullType + '\') FROM DUMMY';
	pcall_seq =  conn.prepareStatement(query);
	pcall_seq.execute();
	var rs = pcall_seq.getResultSet();
	if(rs.next()){
		pre_headerId=String(rs.getString(1));
	}
	return pre_headerId;
}
...
{
	seq_query = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.ui_manual_pull::generate_pull_header_id_m"(?)';
	...
	{			
		...
		//------------Add for manualpulltype -----ZHAODAN1---start--
		MANUALPULLTYPE = data[i].MANUALPULLTYPE;	
		
		if (LAST_PRE_HEADER_ID === '') {
			PRE_HEADER_ID = getPreHeaderId(MANUALPULLTYPE).split(":")[0];
			if (PRE_HEADER_ID === 'NO'){
				error_message = 'Wrong ManualPullType inpute';
				$.response.setBody("Error::"+error_message);
				$.response.status = $.net.http.BAD_REQUEST;
				break;			
			}
			PULL_HEADER_ID = PRE_HEADER_ID + PULL_HEADER_ID;	
		}
		
		LAST_PRE_HEADER_ID = PRE_HEADER_ID;
		
		//------------Add for manualpulltype -----ZHAODAN1---end--
		query = 'CALL "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.ui_manual_pull::createUiManualPull_m"'
			    + '(\''
			    + DEST_STORAGE_LOC+'\',\''
			    + DEST_STORAGE_TYPE+'\',\''
			    + DEST_STORAGE_BIN+'\',\''
			    ...
			    + PULL_HEADER_ID + '\',\''
			    + STATUS + '\',\''
			    + MANUALPULLTYPE + '\'' //ZHAODAN1 20161107
			    + ',?,?)';
		...
		try
		{
			...
			pcall.setString(21,MANUALPULLTYPE);
...
function update_manual_pull()
{
	...
	/*	query = 'CALL "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.ui_manual_pull::editUiManualPull"'
			    ...
			    + STATUS +'\''
			    + ',?,?)';*/
		query = 'CALL "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.ui_manual_pull::editUiManualPull_m"(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
		...
		try
		{
			...
			pcall.setString(22,MANUALPULLTYPE);//ZHAODAN1 20161107
			pcall.execute();
			result_status = pcall.getInteger(23);
			$.response.contentType = "application/json; charset=UTF-8";
			//$.response.setBody("Result Status::"+result_status);
			if(result_status === 0)
			{
				error_message = pcall.getNString(24);
				$.response.setBody("Error::"+error_message);
				$.response.status = $.net.http.BAD_REQUEST;
				break;
			}
