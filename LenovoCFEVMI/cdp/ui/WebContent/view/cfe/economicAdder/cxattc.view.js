...
sap.ui.jsview("lenovo.view.cfe.economicAdder.cxattc", {
...
//			labelLayout: new sap.ui.layout.GridData({span: "L2 M2 S2", linebreak: true}),
			dropdownbox : {
				...
				odata:{
					...
					url: oServiceUrl +"/UI_XATTC_SEARCH_DDL?$filter=ITEM_TYPE eq 'CYCLE'&$format=json"
				}				
			}
		},{
...
//			labelLayout: new sap.ui.layout.GridData({span: "L2 M2 S2", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XATTC_SEARCH_DDL?$filter=ITEM_TYPE eq 'PLANT'&$format=json"
				}	
...
//			labelLayout: new sap.ui.layout.GridData({span: "L3 M3 S3", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XATTC_SEARCH_DDL?$filter=ITEM_TYPE eq 'BRAND'&$format=json"
				}	
...
//			labelLayout: new sap.ui.layout.GridData({span: "L3 M3 S3", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XATTC_SEARCH_DDL?$filter=ITEM_TYPE eq 'SUBGEO'&$format=json"
				}	
...
//			labelLayout: new sap.ui.layout.GridData({span: "L3 M3 S3", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XATTC_SEARCH_DDL?$filter=ITEM_TYPE eq 'TBA_TYPE'&$format=json"
				}	
		...
		//download
		config.download.table = '"_SYS_BIC"."cdp.ebgcfe.models.ui_xattc/CV_UI_XATTC"';
		config.download.columns=[
		                          "CYCLE", "BRAND", "FAMILY", "PLANT", "SUBGEO", "TBA_TYPE", "M1", "M2", "M3", "M4", "M5", 
		                         "M6", "M7", "M8", "M9", "M10", "M11", "M12", "M13", "M14", "M15", "M16", "M17", "M18"];                         
		config.download.filename= "UI_XATTC";
		
		//upload
		config.upload = {
			url: uServiceUrl + "/ui_xattc.xsjs",
			excelUrl: "cfe/economicAdder/xattc.xlsx"
		};
		config.viewstatus = {
			viewUploadHistory: {
				url: uServiceUrl + "/upload.xsodata",
				bindRowUrl: "/UPLOAD_BUSINESS_INFO?$filter=TABLE_NAME eq 'UI_XATTC'",
			...
			viewUploadErrorDetail: {
				url: uServiceUrl + "/upload.xsodata/UI_XATTC_ERR_DETAIL?$format=json",
				columns: [[{
					label: "Cycle",
					field: "CYCLE",
					type:  "TextField"
	...
	createContent: function(){
		...
		var auth = lenovo.control.commontable.Table.getViewAuth("cxattc");
		this.setConfig(config, oServiceUrl, uServiceUrl, auth);

		var header = lenovo.control.commontable.Table.createHeader("Ecomomic Adder", "XATTC");

		config.bindRowUrl = "/UI_XATTC";
...
//		config.create.visible=false;
		...
		lenovo.control.commontable.Toolkit.relateDropDwonBox(brandDropdownBox, prodFamilyDropdownBox, {
			...
			url: function(selectedKey){
				return oServiceUrl+"/INPUT_XATTC(INPUT_BRAND='" + selectedKey + "')/Results?$format=json";
			},
			bindTextField:"ITEM_VALUE",
			bindKeyField:"ITEM_VALUE"
		});
