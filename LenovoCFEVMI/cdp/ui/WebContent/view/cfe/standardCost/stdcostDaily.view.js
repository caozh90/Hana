...
//		cycleDropdownBox.setEnabled(false);
	...
	publishlog: function(oServiceUrl){
		var tablename = '"PCDW"."RSP_STDCOST_DAILY"';
	...
	publish: function(table, oServiceUrl) {
		...
		sap.ui.commons.MessageBox.confirm(oLabel, function(result){
			if(result) {
				 ...
				 *******************************/
				$.ajax({
					url: logicServiceUrl+"/ui_stdcost_daily_publish.xsjs?process_name=CFE_WBSTDCO_DAILY_TO_ECC",
					//data: data,
					type: "get",
					contentType: "text",
	...
	reloadSearchItemDropdownTable: function(filterModel, filterPanel, table){
		...
		var bindUrl = "/UI_STDCOST_DAILY_SEARCH_DDL?";
		table.setBusy(true);
	...
	reloadItemDropdownTable: function(dropdownTable){
		 ...
		 * End by Chris Gao
		 *******************************************************/
		var bindUrl = "/UI_STDCOST_DAILY_SEARCH_DDL?$filter=ITEM_TYPE eq 'ITEM' &$format=json";
