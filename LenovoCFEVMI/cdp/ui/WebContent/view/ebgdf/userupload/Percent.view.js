 ...
 */
/************************************
* Created by bianzh1 at 2015-9-15
...
//main
/******Copy_Change_Start*******/
sap.ui.jsview("lenovo.view.ebgdf.userupload.Percent", { 
/******Copy_Change_End*******/	
	...
	createContent: function(){
		...
		/******Copy_Change_Start*******/
		var auth = lenovo.control.commontable.Table.getViewAuth("Percent");
		/******Copy_Change_End*******/
		...
		/******Copy_Change_Start*******/
		var header = lenovo.control.commontable.Table.createHeader("User Upload", "Class Percent Upload");
		/******Copy_Change_End*******/
		
		//set data connection
		/******Copy_Change_Start*******/
		config.bindRowUrl = "/UI_CLASS_PERCENT";
		config.formWidth = "33.3%";
	...
	setConfig: function(config, oServiceUrl, uServiceUrl, auth) {	
		...
		/******Copy_Change_Start*******/
		config.columns = [
		{field: "ABCD_T", label: "ABCD_T", type:"TextField", width: "200px"},
		{field: "A", label: "A", type:"TextField", width: "80px"},
		{field: "B", label: "B", type:"TextField", width: "80px"},
		{field: "C", label: "C", type:"TextField", width: "80px"},
		{field: "CREATE_TS", label: "Create date", type:"TextField", width: "150px", dateformat: "MM-dd-yyyy"},
		...
		config.filtersRaw = [{
			...
			dropdownbox : {
				...
				odata:{
					...
					url: oServiceUrl +"/UI_CLASS_PERCENT_SEARCH?$filter=ITEM_TYPE eq 'ABCD_T'&$format=json",
			...
			field: "A", label: "A", type: "DropdownBox", 
			labelLayout: new sap.ui.layout.GridData({span: "L2 M2 S2", linebreak: true}),
			dropdownbox : {
				defaultFilterValue: "",
				odata:{
					defaultSelectAll:true,
					url: oServiceUrl +"/UI_CLASS_PERCENT_SEARCH?$filter=ITEM_TYPE eq 'A'&$format=json",
					bindTextField:"ITEM_VALUE",
					bindKeyField:"ITEM_VALUE"
				}
			}
			...
			field: "B", label: "B", type: "DropdownBox", 
			labelLayout: new sap.ui.layout.GridData({span: "L2 M2 S2", linebreak: true}),
			dropdownbox : {
				defaultFilterValue: "",
				odata:{
					defaultSelectAll:true,
					url: oServiceUrl +"/UI_CLASS_PERCENT_SEARCH?$filter=ITEM_TYPE eq 'B'&$format=json",
					bindTextField:"ITEM_VALUE",
					bindKeyField:"ITEM_VALUE"
				}
			}
			...
			field: "C", label: "C", type: "DropdownBox", 
			labelLayout: new sap.ui.layout.GridData({span: "L2 M2 S2", linebreak: true}),
			dropdownbox : {
				defaultFilterValue: "",
				odata:{
					defaultSelectAll:true,
					url: oServiceUrl +"/UI_CLASS_PERCENT_SEARCH?$filter=ITEM_TYPE eq 'C'&$format=json",
					bindTextField:"ITEM_VALUE",
					bindKeyField:"ITEM_VALUE"
				}
			}
		...
		config.insertRaw=[{
			...
			required: true,
			validation: [{
				validType: /[0-9]{1,5}\.[0-9]{2}/,
				errMsg: "The date type of this field is Decimal(4,2)!"
			}],
		},{
			field: "B", 
			label: "B", 
		...
		config.create.url = "/UI_CLASS_PERCENT";
		/******Copy_Change_End*******/
		/******Copy_Change_Start*******/
		//download
		config.download.table = '"_SYS_BIC"."cdp.ebgdf.models.ui_class_percent/CV_UI_CLASS_PERCENT"';
		config.download.columns=[
			"ABCD_T","A","B","C","CREATE_TS","CREATE_USERID","LAST_UPDATE_TS","LAST_UPDATE_USERID"];

		config.download.filename= "Percent";
		/******Copy_Change_End*******/
		...
		//upload
		config.upload = {
			url: uServiceUrl + "/ui_class_percent.xsjs",
			excelUrl: "ebgdf/percent/Percent.xlsx"
		};
		//status view
		config.viewstatus = {
				viewUploadHistory: {
					url: uServiceUrl + "/upload.xsodata",
					bindRowUrl: "/UPLOAD_BUSINESS_INFO?$filter=TABLE_NAME eq 'UI_CLASS_PERCENT'",
				...
				viewUploadErrorDetail: {
					url: uServiceUrl + "/upload.xsodata/UI_CLASS_PERCENT_ERR_DETAIL?$format=json",
					columns: [[{
						field: "ABCD_T",
						label: "ABCD_T",
						type:  "TextField"
	...
	deleteAll: function(table){
		sap.ui.commons.MessageBox.confirm("Do you want to delete all items?", function(result){
			if(result){
				var service = new lenovo.service.EBGDF();
				var logicServiceUrl = service.getebgdfLogic();
				var table_name = "UI_CLASS_PERCENT";
				var obj = {
