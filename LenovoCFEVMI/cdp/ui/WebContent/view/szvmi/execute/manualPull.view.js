...
sap.ui.jsview(
				"lenovo.view.szvmi.execute.manualPull",
				{
					...
					setConfig : function(config, sServiceUrl, uServiceUrl,
						...
						config.bindRowUrl = "/UI_MANUAL_PULL";// ?$orderby=PULL_HEADER_ID
						...
						config.filtersRaw = [
								...
								},
								{
									...
									dropdownbox : {
										odata : {
											url : sServiceUrl
													+ "/InputParams(P_PTYPE='TPL_ID')/Results?$format=json",
											bindKeyField : "PVALUE",
								...
								},
								{
									...
									dropdownbox : {
										odata : {
											url : sServiceUrl
													+ "/InputParams(P_PTYPE='INVENTORY_TYPE')/Results?$format=json",
											bindKeyField : "PVALUE",
											bindTextField : "PVALUE",
											defaultSelectAll : true
										}
								...
								},
								{
									...
									dropdownbox : {
										odata : {
											url : sServiceUrl
													+ "/CONF_SITEMASTER?$select=SITEID&$format=json",
											bindKeyField : "SITEID",
						...
						config.download.columns = [ "PULL_HEADER_ID",
								"PULL_LINE_ID", "MATNR", "MATNR_DESCR",
								"SRC_WERKS", "SRC_STORAGE_LOC",
								"SRC_STORAGE_TYPE", "SRC_STORAGE_BIN",
								"PULL_TYPE", "DEST_STORAGE_LOC",
								"DEST_STORAGE_TYPE", "DEST_STORAGE_BIN",
								"INVENTORY_TYPE", "LINE_SUPPLIERID",
								"PULL_QTY", "HEAD_SUPPLIERID", "REMARK",
								"STATUS", "DELIVERY_DATE", "PRODUCTION_LINE",
								"PHYSICAL_PLANT", "SYS_CREATED_DATE",
								"SYS_CREATED_BY", "SYS_LAST_MODIFIED_BY",
						...
						config.upload = {
							...
							excelUrl : "szvmi/execute/manual_pull.xlsx"
						};
					...
					createContent : function(oController) {
						...
						var header = lenovo.control.commontable.Table
								.createHeader("execute", "manual pull");
					...
					_deleteAllService : function(iServiceUrl, table) {
						...
						$.ajax({
							url : iServiceUrl
									+ "/ui_manual_pull_delete_all_draft.xsjs",
							type : "post",
							dataType : "text",
							async : false,
					...
					_openInsertDialog : function(sServiceUrl, config, objects,
						...
						var dialog = new Dialog("insert", {
							title : objects == undefined ? "Create Pull"
									: "Edit Pull",
							keepInWindow : true,
							height : "600px",
							modal : true,
					...
					_createContents : function(url, objects) {
						var that = this;
						// LOGICAL_PLANT
						var lLogicalP = new sap.ui.commons.Label({
							text : 'Logical Plant:',
							width : "110px"
						});
						...
						iLogicalP.bindItems("/CONF_SITEMASTER?$select=SITEID&$format=json",
								oItemTemplate1);
						iLogicalP.setValue(objects == undefined ? ""
								: objects[0].SRC_WERKS);
						
						
						// INVENTORY_TYPE
						var lInvType = new sap.ui.commons.Label({
							text : 'Inventory Type:',
							width : "110px"
						});
						var iInvType = new sap.ui.commons.DropdownBox(
								"iInvType", {
									width : "142px"
								}).addStyleClass("requiredDropdown")
								.attachChange(
										function() {
											var soi = sap.ui.getCore().byId(
													"i3PLId2");
											var loi = sap.ui.getCore().byId(
													"i3PLId");
											var layout = sap.ui.getCore().byId(
													"header1");
						...
						iInvType
								.bindItems(
										"/InputParams(P_PTYPE='INVENTORY_TYPE')/Results",
										oItemTemplate1);
						iInvType.setValue(objects == undefined ? ""
								: objects[0].INVENTORY_TYPE);
						...
						;
						iHeadId.setValue(objects == undefined ? ""
								: objects[0].PULL_HEADER_ID);

						// HEAD_SUPPLIERID
						var l3PLId = new sap.ui.commons.Label({
							text : '3PL ID:',
							width : "110px"
						});
						...
						oItemTemplate1.bindProperty("text", "PVALUE");
						i3PLId.bindItems(
								"/InputParams(P_PTYPE='LOI_TPL_ID')/Results",
								oItemTemplate1);
						...
						oItemTemplate1.bindProperty("text", "PVALUE");
						i3PLId2.bindItems(
								"/InputParams(P_PTYPE='SOI_TPL_ID')/Results",
								oItemTemplate1);

						var oLayout1 = new sap.ui.layout.HorizontalLayout(
								"header1", {
									content : [ lLogicalP, iLogicalP, lInvType,
											iInvType, l3PLId, iHeadId ]
								// /i3PLId,
								}).addStyleClass("header");

						if (objects != undefined
								&& objects[0].INVENTORY_TYPE == "SOI") {
							oLayout1.insertContent(i3PLId2, 5);
							i3PLId2.setValue(objects[0].HEAD_SUPPLIERID);
						} else {
							oLayout1.insertContent(i3PLId, 5);
							i3PLId.setValue(objects == undefined ? ""
									: objects[0].HEAD_SUPPLIERID);
						}

						// DEST_STORAGE_TYPE
						var lDestType = new sap.ui.commons.Label({
							text : 'Destination Type:',
							width : "112px"
						});
						/************************
						 * Modified by Chris Gao
						 * to disabled the value help
						 * use extend lenovo.control.LenovoValueHelpField
						 * 2015-09-18
						 *************************/
						var iDestType = new lenovo.control.LenovoValueHelpField(
								"iDestType", {
									placeholder: "--selection only--",
									width : "142px",
									valueHelpRequest : function(oEvent) {
										that._openSearchHelpForDest(url, this);
									}
								}).addStyleClass("required2").attachChange(
								that._isRequired);//added by Chris Gao 2015-09-18
						iDestType.setValue(objects == undefined ? ""
								: objects[0].DEST_STORAGE_TYPE);
						
						/************************
						 * End by Chris Gao 2015-09-18
						 *************************/
					
						
						// DEST_STORAGE_BIN
						var lDestBin = new sap.ui.commons.Label({
						...
						iDestBin.setValue(objects == undefined ? ""
								: objects[0].DEST_STORAGE_BIN);
						// DEST_STORAGE_LOC
						var lDestStorLoc = new sap.ui.commons.Label({
							text : 'Dest Stor Loc:',
							width : "110px"
						});
						var iDestStorLoc = new sap.ui.commons.TextField(
								"iDestStorLoc", {
									width : "142px",
									// editable: false,
									enabled : false
						...
						iDestStorLoc.setValue(objects == undefined ? ""
								: objects[0].DEST_STORAGE_LOC);
						var oLayout2 = new sap.ui.layout.HorizontalLayout(
								"header2", {
									content : [ lDestType, iDestType, lDestBin,
											iDestBin, lDestStorLoc,
											iDestStorLoc ]
								}).addStyleClass("header");
						...
						// PRODUCTION_LINE
						var lProductLine = new sap.ui.commons.Label({
							text : 'Production Line:',
							width : "110px"
						});
						var iProductLine = new lenovo.control.LenovoValueHelpField(
								"iProductLine", {
									placeholder: "--selection only--",
									width : "142px",
									valueHelpRequest : function(oEvent) {
										that._openSearchHelpForProductionLine(
												url, this);
									}
								});
						iProductLine.setValue(objects == undefined ? ""
								: objects[0].PRODUCTION_LINE);
						// PHYSICAL_PLANT
						var lPhysicalPlant = new sap.ui.commons.Label({
							text : 'Physical Plant:',
							width : "110px"
						});
						var iPhysicalPlant = new lenovo.control.LenovoValueHelpField(
								"iPhysicalPlant", {
									placeholder: "--selection only--",
									width : "142px",
									valueHelpRequest : function(oEvent) {
										that._openSearchHelpForPhysicalPlant(
												url, this);
									}
								});
						iPhysicalPlant.setValue(objects == undefined ? ""
								: objects[0].PHYSICAL_PLANT);

						// PULL_TYPE
						var lPullType = new sap.ui.commons.Label({
							text : 'Pull Type:',
							width : "110px"
						});
						var iPullType = new sap.ui.commons.TextField(
								"iPullType", {
									width : "142px",
									editable : false,
								// enabled: objects==undefined?true:false
								});
						iPullType.setValue(objects == undefined ? "MANUAL"
								: objects[0].PULL_TYPE);
						var oLayout3 = new sap.ui.layout.HorizontalLayout(
								"header3", {
									content : [ lProductLine, iProductLine,
											lPhysicalPlant, iPhysicalPlant,
											lPullType, iPullType ]
								}).addStyleClass("header");
						// DELIVERY_DATE
						var lDeliveryDate = new sap.ui.commons.Label({
							text : 'Delivery Date:',
							width : "110px"
						});
						var iDeliveryDate = new sap.ui.commons.DatePicker(
								"iDeliveryDate", {
									width : "142px",
									value:{
										type: new sap.ui.model.type.Date({pattern: "yy-MM-dd hh:mm:ss", strictParsing: true})
									}
								});
						iDeliveryDate
								.setValue(objects == undefined ? (new Date())
										.Format("yyyy-MM-dd hh:mm:ss") : DateConvert(
										objects[0].DELIVERY_DATE).Format(
										"yyyy-MM-dd hh:mm:ss"));
						...
						var iPartNum = new lenovo.control.LenovoValueHelpField("iPartNum",
								{   placeholder: "--selection only--",
									width : "142px",									
									valueHelpRequest : function(oEvent) { //zhaodan1
						...
						var lPullQty = new sap.ui.commons.Label({
							text : "Pull Qty",
							width : "110px"
						});
						var iPullQty = new sap.ui.commons.TextField("iPQ", {
							width : "142px"
						}).addStyleClass("required2").attachChange(
								that._isRequired);
						var lSourceType = new sap.ui.commons.Label({
							text : "Source Type",
							width : "110px"
						});
						var iSourceType = new lenovo.control.LenovoValueHelpField(
								"iST", {
									placeholder: "--selection only--",
									width : "142px",
									valueHelpRequest : function(oEvent) {
										that._openSearchHelpForSourceType(url,
												this);
									}
								}).addStyleClass("required2").attachChange(
								that._isRequired);

						var iRSL = new sap.ui.commons.TextField("iSSL", {
							width : "142px",
						}).addStyleClass("hiddenStyle");
						var iRB = new sap.ui.commons.TextField("iSB", {
							width : "142px",
						}).addStyleClass("hiddenStyle");
						var lSupplier = new sap.ui.commons.Label({
							text : "Supplier",
							width : "110px"
						});
						var iSupplier = new lenovo.control.LenovoValueHelpField(
								"iSupplier", {
								    enabled: false,
									placeholder: "--selection only--",
									width : "142px",
									valueHelpRequest : function(oEvent) {
										that._openSearchHelpForSupplier(url,
												this);
									}
						...
						var oLayout5 = new sap.ui.layout.HorizontalLayout({
							content : [ lPartNum, iPartNum, lPullQty, iPullQty,
									lSourceType, iSourceType ]
						});
						var oLayout6 = new sap.ui.layout.HorizontalLayout({
							content : [ lSupplier, iSupplier, iRSL, iRB,
									oAddPartButton ]
						});
						...
						var oLayout = new sap.ui.layout.VerticalLayout({
							content : [ oLayout1, oLayout2, oLayout3, oLayout4,
									oPanel, oPanel1 ]
						});
					...
					_onAddPart : function() {
						...
						var url = "/INPUT_VALIDATION(IN_PART_NUMBER='"
								+ s.byId("iPartNum").getValue()
								+ "',IN_LOGIC_PLANT='"
									...
									success : function(resp) {
										if (resp > 0) {
											...
											oEntry.MATNR = s.byId("iPartNum")
													.getValue();
											oEntry.LINE_SUPPLIERID = s.byId(
													"iInvType").getValue() == "SOI" ? s
													.byId("iSupplier")
													.getValue()
													: "3PL";
											oEntry.SRC_STORAGE_TYPE = s.byId(
													"iST").getValue();
											oEntry.SRC_STORAGE_LOC = s.byId(
													"iSSL").getValue();
											oEntry.SRC_STORAGE_BIN = s.byId(
													"iSB").getValue();
											oEntry.PULL_QTY = s.byId("iPQ")
													.getValue();
											oEntry.STATUS = "DRAFT";
											oEntry.REMARK = "";
											aData.push(oEntry);
					...
					_openSearchHelpForDest : function(url, oControl) {
						...
						var oModel = new sap.ui.model.odata.ODataModel(url,
								true);
						//added by Chris Gao 2015-09-22 -- to clear count error
						oModel.setDefaultCountMode("None");
					...
					_openSearchHelpForPartNum : function(url, oControl) { //zhaodan1
						var that = this;
						var s = sap.ui.getCore();
//
//						var tpl = "";
...
//						}
					...
					_openSearchHelpForSupplier : function(url, oControl) {
						...
						var dropdownTableConfig = {
							defaultFilterOp : "EQ",
							url : url,
							bindRowUrl : "/SUPITEM_PULL?$format=json&$filter=PARTNO eq '" + iPartNum + "'",//zhaodan1
					...
					_onEdit : function(table, config, sServiceUrl, iServiceUrl) {
						...
						if (selectedObject.STATUS == "DRAFT") {
							...
							var url = sServiceUrl
									+ config.bindRowUrl
									+ "?$format=json&$filter=PULL_HEADER_ID eq '"
									+ selectedObject.PULL_HEADER_ID + "'";
					...
					_createPullLineTable : function(objects) {
						var that = this;
						var service = new lenovo.service.SZVMI;//zhaodan1
						var sServiceUrl = service.getMXVmi();//zhaodan1
											...
											new sap.ui.table.Column(
													{
														...
														/*template : new sap.ui.commons.TextField(
																{
																	editable : false zhaodan1
																})*/
														template : new lenovo.control.LenovoValueHelpField(
																{
																	//editable : true,
																	placeholder: "--selection only--",
																	valueHelpRequest : function(oEvent) {
					...
					_onCreate : function(dialog, iServiceUrl, config, status,
						...
						data.DEST_STORAGE_LOC = s.byId("iDestStorLoc")
								.getValue();
						...
						data.PHYSICAL_PLANT = s.byId("iPhysicalPlant")
								.getValue();

						var invType = s.byId("iInvType").getValue();
						data.INVENTORY_TYPE = invType;
						data.PRODUCTION_LINE = s.byId("iProductLine")
								.getValue();
						if (invType == "SOI") {
						...
						// data.HEAD_SUPPLIERID = s.byId("i3PLId").getValue();
						data.DELIVERY_DATE = (new Date(s.byId("iDeliveryDate")
								.getValue())).Format("yyyy-MM-dd hh:mm:ss");//added by Chris Gao 2015-09-22
