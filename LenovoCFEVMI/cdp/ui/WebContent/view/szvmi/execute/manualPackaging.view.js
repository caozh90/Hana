/* 
	develop by Coral Zhang @ 2014/12/30
*/
jQuery.sap.require("lenovo.control.commontable.Table");
jQuery.sap.require("lenovo.control.Validation");
jQuery.sap.require("lenovo.service.SZVMI");
jQuery.sap.require("lenovo.control.LenovoValueHelpField");//zhaodan1

sap.ui.jsview("lenovo.view.szvmi.execute.manualPackaging", {
	...
	setConfig: function(config, sServiceUrl,uServiceUrl,dSchema) {
		...
		var auth =  lenovo.control.commontable.Table.getViewAuth("manualPackaging");
		...
		config.filtersRaw = [{
			...
			dropdownbox: {
				odata: {
					url: sServiceUrl + "/InputParams(P_PTYPE='SOI_TPL_ID')/Results?$format=json",
					bindKeyField: "PVALUE",
			...
			textfield: {
				...
				defaultFilterOp: "EQ",
				enabled: false,
				defaultFilterValue: "MANUAL_PACKAGING"
			}
		}, {
			field: "STATUS",
			label: "Status",
	...
	createContent: function(oController) {
		...
		var header = lenovo.control.commontable.Table.createHeader("execute", "Manual Packaging");
	...
	onTreeNavigation: function(sChannel, sEvent, oData){
		if(oData.view === "Manual Packaging") {
	...
	_deleteAllService: function(iServiceUrl, table){
		var sendData = {};
		sendData.PULL_TYPE = "MANUAL_PACKAGING";
		table.setBusy(true);
		$.ajax({
	...
	_createContents: function(url, objects){
		...
		var i3PLId = new sap.ui.commons.DropdownBox("i3PLId", {
			width: "142px"
		}).addStyleClass("requiredDropdown").attachChange(that._isRequired);
		var oModel = new sap.ui.model.odata.ODataModel(url, true);
		i3PLId.setModel(oModel);
		var oItemTemplate1 = new sap.ui.core.ListItem();
		oItemTemplate1.bindProperty("text", "PVALUE");
		i3PLId.bindItems("/InputParams(P_PTYPE='SOI_TPL_ID')/Results", oItemTemplate1);
		...
		iProductLine.bindItems("/UI_LINE_FLOOR_MAPPING", oItemTemplate1);
		...
		iPullType.setValue(objects==undefined?"MANUAL_PACKAGING":objects[0].PULL_TYPE);
		...
		var iPartNum = new lenovo.control.LenovoValueHelpField("iPartNum",
				...
				}).addStyleClass("required").attachChange(
				that._isRequired);	
		...
		var iSourceType = new lenovo.control.LenovoValueHelpField("iST",{ //zhaodan1
			placeholder: "--selection only--",
			width: "142px",
		...
		var iSupplier = new lenovo.control.LenovoValueHelpField("iSupplier",{//zhaodan1
			placeholder: "--selection only--",
			width: "142px",
		...
		var oLayout = new sap.ui.layout.VerticalLayout({
			content: [oLayout1, oLayout2, oLayout3, oLayout4, oHD, oPanel, oHD, oPanel1]
		});
...
//		}
	...
	_openSearchHelpForSourceType: function(url, oControl){
		var that = this;
		var s = sap.ui.getCore();
		//var bindRowUrl = "/CONF_WAHREHOUSE_INFO?$filter=(TPL_ID eq '" + s.byId("i3PLId").getValue() + "')and(TYPE eq '" + s.byId("iInvType").getValue() + "')and(LOGIC_PLANT eq '" + s.byId("iLogicalP").getValue() + "')and(IS_SOURCE eq 'Y')";
		var bindRowUrl = "/INPUT_CONF_WAHREHOUSE_INFO(P_WHERE='"
			+"(\"LOGIC_PLANT\" =''"+ s.byId("iLogicalP").getValue()+"'') AND ("
			+ "\"IS_SOURCE\" =''Y'') AND ("
			+"\"TYPE\" =''"+s.byId("iInvType").getValue()+"'') AND ("
			+"\"TPL_ID\" =''" + s.byId("i3PLId").getValue() + "'') "
			+"')/Results";
	...
	_openSearchHelpForDest: function(url, oControl, isDest){
		...
		/*var bindRowUrl = "/CONF_WAHREHOUSE_INFO?$filter=(LOGIC_PLANT eq '" + s.byId("iLogicalP").getValue() +"')"
						// + "and(TYPE eq '" + s.byId("iInvType").getValue() + "')"
						// + "and(TPL_ID eq '" + s.byId("i3PLId").getValue() + "')"
						+ "and(IS_DEST eq 'Y')";
*/
		var bindRowUrl = "/INPUT_CONF_WAHREHOUSE_INFO(P_WHERE='"
			+"(\"LOGIC_PLANT\" =''"+ s.byId("iLogicalP").getValue()+"'') AND ("
	...
	_openSearchHelpForSupplier: function(url, oControl){
		...
		var dropdownTableConfig = {
			defaultFilterOp: "EQ",
			url: url,
			bindRowUrl : "/SUPITEM?$format=json&$filter=PARTNO eq '" + iPartNum + "'",//zhaodan1
			fields: [["SUPPLIERID",oControl]],
			columns: [{
				label: "Supplier Id",
				field: "SUPPLIERID",
		...
		var oTable = new sap.ui.table.Table("pullLineInfo", {
			...
			columns: [
				...
				new sap.ui.table.Column({
					...
					template : new lenovo.control.LenovoValueHelpField( //zhaodan1
							{
								//editable : true,
								placeholder: "--selection only--",
		...
		$.ajax({
			...
			success: function(resp){
				if(resp > 0){
					...
					oEntry.LINE_SUPPLIERID = s.byId("iSupplier").getValue();
