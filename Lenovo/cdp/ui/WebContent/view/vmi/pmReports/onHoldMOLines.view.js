...
sap.ui.jsview("lenovo.view.vmi.pmReports.onHoldMOLines", {
	setConfig: function(config,sServiceUrl) {
		...
		config.filtersRaw = [{
			...
			dropdowntable: {
				defaultFilterOp: "EQ",
				url: sServiceUrl,
				bindRowUrl: "/ON_HOLD_MO_LINES_DDL?$format=json&$select=SYS_TIMESTAMP,MS&$orderby=SYS_TIMESTAMP desc",
				field: "SYS_TIMESTAMP",
				columns: [{
					label: "Sys Timestamp",
					field: "SYS_TIMESTAMP",
		...
		config.download.table = '"_SYS_BIC"."cdp.mxebgvmi.models.pmreport/CV_ON_HOLD_MO_LINES"';
