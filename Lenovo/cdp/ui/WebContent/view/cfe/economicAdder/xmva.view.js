...
sap.ui.jsview("lenovo.view.cfe.economicAdder.xmva", {
...
//			labelLayout: new sap.ui.layout.GridData({span: "L2 M2 S2", linebreak: true}),
			dropdownbox : {
				...
				odata:{
					...
					url: oServiceUrl +"/UI_XMVA_SEARCH_DDL?$filter=ITEM_TYPE eq 'CYCLE'&$format=json"
				}				
			}
		},{
...
//			labelLayout: new sap.ui.layout.GridData({span: "L2 M2 S2", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XMVA_SEARCH_DDL?$filter=ITEM_TYPE eq 'PLANT'&$format=json"
				}	
...
//			labelLayout: new sap.ui.layout.GridData({span: "L3 M3 S3", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XMVA_SEARCH_DDL?$filter=ITEM_TYPE eq 'BRAND'&$format=json"
				}	
...
//			labelLayout: new sap.ui.layout.GridData({span: "L3 M3 S3", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XMVA_SEARCH_DDL?$filter=ITEM_TYPE eq 'SUBGEO'&$format=json"
				}	
...
//			labelLayout: new sap.ui.layout.GridData({span: "L3 M3 S3", linebreak: true}),
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_XMVA_SEARCH_DDL?$filter=ITEM_TYPE eq 'TBA_TYPE'&$format=json"
				}	
		...
		//download
		config.download.table = '"_SYS_BIC"."cdp.ebgcfe.models.ui_xmva/CV_UI_XMVA"';
		config.download.columns=[
		                          "CYCLE", "BRAND", "FAMILY", "PLANT", "SUBGEO", "TBA_TYPE", "M1", "M2", "M3", "M4", "M5", 
		                         "M6", "M7", "M8", "M9", "M10", "M11", "M12", "M13", "M14", "M15", "M16", "M17", "M18"];
		config.download.filename= "UI_XMVA";
		
		//upload
		config.upload = {
			url: uServiceUrl + "/ui_xmva.xsjs",
			excelUrl: "cfe/economicAdder/xmva.xlsx"
		};
		config.viewstatus = {
			viewUploadHistory: {
				url: uServiceUrl + "/upload.xsodata",
				bindRowUrl: "/UPLOAD_BUSINESS_INFO?$filter=TABLE_NAME eq 'UI_XMVA'",
			...
			viewUploadErrorDetail: {
				url: uServiceUrl + "/upload.xsodata/UI_XMVA_ERR_DETAIL?$format=json",
				columns: [[{
					label: "Cycle",
					field: "CYCLE",
					type:  "TextField"
	...
	createContent: function(){
		...
		var auth = lenovo.control.commontable.Table.getViewAuth("xmva");
		this.setConfig(config, oServiceUrl, uServiceUrl, auth);
		
		var header = lenovo.control.commontable.Table.createHeader("Ecomomic Adder", "XMVA");

		config.bindRowUrl = "/UI_XMVA";
		...
		lenovo.control.commontable.Toolkit.relateDropDwonBox(brandDropdownBox, prodFamilyDropdownBox, {
			...
			url: function(selectedKey){
				return oServiceUrl+"/INPUT_XMVA(INPUT_BRAND='" + selectedKey + "')/Results?$format=json";
			},
			bindTextField:"ITEM_VALUE",
			bindKeyField:"ITEM_VALUE"
		});
