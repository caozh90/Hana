...
sap.ui.jsview("lenovo.view.cfe.economicAdder.wwvar", {
		...
		config.filtersRaw = [{
			...
			dropdownbox : {
				...
				odata:{
					...
					url: oServiceUrl +"/UI_WWVAR_SEARCH_DDL?$filter=ITEM_TYPE eq 'CYCLE'&$format=json"
				}				
			}
		},{
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_WWVAR_SEARCH_DDL?$filter=ITEM_TYPE eq 'SUBGEO'&$format=json"
				}	
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_WWVAR_SEARCH_DDL?$filter=ITEM_TYPE eq 'BRAND'&$format=json"
				}	
			...
			dropdowntable: {
				defaultFilterOp: "EQ",
				url: oServiceUrl,
				bindRowUrl: "/WWVAR_SEARCH_ASSEMBLY",
...
//						defaultFilterOp: "EQ"
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_WWVAR_SEARCH_DDL?$filter=ITEM_TYPE eq 'ASSEMBLY'&$format=json"
				}	
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_WWVAR_SEARCH_DDL?$filter=ITEM_TYPE eq 'TBA_TYPE'&$format=json"
				}	
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_WWVAR_SEARCH_DDL?$filter=ITEM_TYPE eq 'PLANT'&$format=json"
				}	
		...
		//download
		config.download.table = '"_SYS_BIC"."cdp.ebgcfe.models.ui_wwvar/CV_UI_WWVAR"';
		config.download.columns=[
		                          "CYCLE", "BRAND", "FAMILY", "PLANT", "SUBGEO", "TBA_TYPE","ASSEMBLY", "M1", "M2", "M3", "M4", "M5", 
		                         "M6", "M7", "M8", "M9", "M10", "M11", "M12", "M13", "M14", "M15", "M16", "M17", "M18"];		                        
		config.download.filename= "UI_WWVAR";
		
		//upload
		config.upload = {
			url: uServiceUrl + "/ui_wwvar.xsjs",
			excelUrl: "cfe/economicAdder/wwvar.xlsx"
		};
		config.viewstatus = {
			viewUploadHistory: {
				url: uServiceUrl + "/upload.xsodata",
				bindRowUrl: "/UPLOAD_BUSINESS_INFO?$filter=TABLE_NAME eq 'UI_WWVAR'",
				selectionMode: sap.ui.table.SelectionMode.Single,
				visibleRowCount: 10
			},
			viewErrorInfo: {
			...
			viewUploadErrorDetail: {
				url: uServiceUrl + "/upload.xsodata/UI_WWVAR_ERR_DETAIL?$format=json",
				columns: [[{
					label: "Cycle",
					field: "CYCLE",
					type:  "TextField"
...
//						defaultFilterOp: "EQ"
	...
	createContent: function(){
		...
		var auth = lenovo.control.commontable.Table.getViewAuth("wwvar");
		this.setConfig(config, oServiceUrl, uServiceUrl, auth);
		
		var header = lenovo.control.commontable.Table.createHeader("Ecomomic Adder", "WWVAR");

		config.bindRowUrl = "/UI_WWVAR";
...
//		config.create.visible=false;
		...
		lenovo.control.commontable.Toolkit.relateDropDwonBox(brandDropdownBox, prodFamilyDropdownBox, {
			...
			url: function(selectedKey){
				return oServiceUrl+"/INPUT_WWVAR(INPUT_BRAND='" + selectedKey + "')/Results?$format=json";
			},
			bindTextField:"ITEM_VALUE",
			bindKeyField:"ITEM_VALUE"
		});
