...
sap.ui.jsview("lenovo.view.cfe.CommissionableAdder.frtPh", {
...
//		var headerHeight = 255;
		...
		config.filtersRaw = [{
			...
			dropdownbox : {
				...
				odata:{
					...
					url: oServiceUrl +"/UI_FRT_PH_SEARCH_DDL?$filter=ITEM_TYPE eq 'CYCLE'&$orderby=ITEM_VALUE desc&$format=json"
				}				
			}
		},{
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_FRT_PH_SEARCH_DDL?$filter=ITEM_TYPE eq 'PLANT'&$format=json"
				}	
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_FRT_PH_SEARCH_DDL?$filter=ITEM_TYPE eq 'COUNTRY'&$format=json"
				}
			}
			...
			dropdownbox : {
				odata:{
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_FRT_PH_SEARCH_DDL?$filter=ITEM_TYPE eq 'TBA_TYPE'&$format=json"
				}	
			}
		}];
		...
		//download
		config.download.table = '"_SYS_BIC"."cdp.ebgcfe.models.ui_frt_ph/CA_UI_FRT_PH"';
		...
		config.download.filename= "UI_FRT_PH";
		
		//upload
		config.upload = {
			visible: true,
			url: uServiceUrl + "/ui_frt_ph.xsjs",
			excelUrl: "cfe/CommissionableAdder/frt_ph.xlsx"
		};
		config.viewstatus = {
			visible: true,
			viewUploadHistory: {
				url: uServiceUrl + "/upload.xsodata",
				bindRowUrl: "/UPLOAD_BUSINESS_INFO?$filter=TABLE_NAME eq 'UI_FRT_PH'",
				selectionMode: sap.ui.table.SelectionMode.Single,
				visibleRowCount: 10
			},
			viewErrorInfo: {
			...
			viewUploadErrorDetail: {
				url: uServiceUrl + "/upload.xsodata/UI_FRT_PH_ERR_DETAIL?$format=json",
				columns: [[{
					label: "Cycle",
					field: "CYCLE",
					type:  "TextField"
		...
		config.insertRaw=[{
			...
			dropdownbox : {
				odata:{
					defaultSelectAll: true,
					url: oServiceUrl +"/UI_FRT_PH_CREATE_DDL?$filter=ITEM_TYPE eq 'PLANT'&$format=json",
					bindTextField:"ITEM_VALUE",
					bindKeyField:"ITEM_VALUE"
				}
			}
			...
			dropdownbox : {
				odata:	{			
					...
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE",
					url: oServiceUrl +"/UI_FRT_PH_CREATE_DDL?$filter=ITEM_TYPE eq 'COUNTRY'&$format=json"
				}
			}
		},{
			...
			dropdownbox : {
				odata:{
					defaultSelectAll: true,
					url: oServiceUrl +"/UI_FRT_PH_CREATE_DDL?$filter=ITEM_TYPE eq 'TBA_TYPE'&$format=json",
					bindTextField:"ITEM_VALUE",
					bindKeyField:"ITEM_VALUE"
				}
			}
	...
	reloadCreateSearchItemDropdownTable: function(filterModel, filterPanel, table){
		...
		var bindUrl = "/UI_FRT_PH_CREATE_DDL?";
		table.setBusy(true);
		table.clearSelection();
		table.bindRows(bindUrl,  null, null, filterModel);
	},
	...
	reloadCreateItemDropdownTable: function(dropdownTable){
		 ...
		 * End by Chris Gao
		 *******************************************************/
		var bindUrl = "/UI_FRT_PH_CREATE_DDL?$filter=ITEM_TYPE eq 'ITEM' &$format=json";
		dropdownTable.setBusy(true);
		dropdownTable.clearSelection();
		dropdownTable.bindRows(bindUrl);
	},	
	createContent: function(){
		...
		var auth = lenovo.control.commontable.Table.getViewAuth("frtPh");
		
		this.setConfig(config, oServiceUrl, uServiceUrl,auth);
		var header = lenovo.control.commontable.Table.createHeader("Commissionable Adder", "FRT by PH");

		config.bindRowUrl = "/UI_FRT_PH";
