...
//main
/******Copy_Change_Start*******/
sap.ui.jsview("lenovo.view.cfe.report.rptStdcostMonthly", { 
/******Copy_Change_End*******/	
	...
	createContent: function(){
		...
		/******Copy_Change_Start*******/
		var auth = lenovo.control.commontable.Table.getViewAuth("rptStdcostMonthly");
		/******Copy_Change_End*******/
		...
		/******Copy_Change_Start*******/
		var header = lenovo.control.commontable.Table.createHeader("Report", "Monthly StdCost Report");
		/******Copy_Change_End*******/
		
		//set data connection
		/******Copy_Change_Start*******/
		//INPUT_CYCLE_ITEM(in_cycle='CURRENT', in_partnumber='000000C25127,000000C25128')/Results
		config.bindRowUrl = "/RPT_STDCOST_MONTHLY";
		/******Copy_Change_End*******/
				     ...
				     [{
						...
						dropdownbox : {
							...
							odata:{
								...
								url: oServiceUrl+"/RPT_STDCOST_MONTHLY_SEARCH_DDL?$filter=ITEM_TYPE eq 'CYCLE' &$format=json"
							}				
						}
				     }],  
				     [{
		...
		var oDownloadTemplateButton = new sap.ui.commons.Button({
			...
			press: function() {
				window.open(lenovo.control.Constants.uploadTemplatesBaseUrl + "cfe/report/stdcost_monthly_import_execute.xlsx");
			}
	...
	//			selectionMode: sap.ui.table.SelectionMode.Single,
			...
			dropdownbox : {
				...
				odata:{
					...
					url: oServiceUrl +"/RPT_STDCOST_MONTHLY_SEARCH_DDL?$filter=ITEM_TYPE eq 'PLANT'&$format=json",
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE"
				}				
			}
			...
			dropdownbox : {
				...
				odata:{
					defaultSelectAll: true,
					url: oServiceUrl +"/RPT_STDCOST_MONTHLY_SEARCH_DDL?$filter=ITEM_TYPE eq 'ITEM_TYPE'&$format=json",
					bindTextField: "ITEM_VALUE",
					bindKeyField: "ITEM_VALUE"
				}				
			}
		...
		//import
		config.import = {
				url: uServiceUrl + "/ui_monthly_stdcost_report.xsjs"
			};
		
		/******Copy_Change_Start*******/
		//download
		//download
		config.download.table = '"_SYS_BIC"."cdp.ebgcfe.models.rpt_stdcost_monthly/CV_RPT_STDCOST_MONTHLY"';
		/******************************
		 * Added by cui yue
		 * 2015-09-29
		 ...
		 * End by cui yue
		 ****************************/
		config.download.filename= "RPT_STDCOST_MONTHLY";
	...
	reloadItemDropdownTable: function(dropdownTable){
		 ...
		 * End by Chris Gao
		 *******************************************************/
		var bindUrl = "/RPT_STDCOST_MONTHLY_SEARCH_DDL?$filter=ITEM_TYPE eq 'ITEM'&$format=json";
	...
	reloadSearchItemDropdownTable: function(filterModel, filterPanel, table){
		...
		var bindUrl = "/RPT_STDCOST_MONTHLY_SEARCH_DDL?";
		table.setBusy(true);
	...
	reloadExecutedItemDropdownTable: function(dropdownTable){
		...
		var bindUrl = "/RPT_STDCOST_MONTHLY_ITEM_DDL";
		dropdownTable.setBusy(true);
		dropdownTable.clearSelection();
		dropdownTable.bindRows(bindUrl);
	},
		...
		sap.ui.commons.MessageBox.confirm(oLabel, function(result){
			if(result) {
				...
				var data = {
						...
						"process_name": 'PRC_RPT_STDCOST_MONTHLY'
				};
				var service = new lenovo.service.CFE();
