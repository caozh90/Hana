...
/*1. get data from UI*/
var process_names = String($.request.parameters.get("process_name"));
var process_name_array = JSON.parse(process_names);
...
for(i = 0; i < process_name_array.length; i++)
{
	...
	/*4 get DS job info*/
	sub_query = 'select * from "_SYS_BIC"."cdp.ds.models/AT_HANA_DS_JOB_INFO" where PROCESS_NAME=\''+process_name_array[i].toUpperCase()+'\'';
...
//			  + parameters
		...
		result_Message = result_Message + process_name_array[i].toUpperCase() + ': ' +run_job() + '\n';
...
//			  + parameters 
	...
	{
		result_Message = result_Message + process_name_array[i].toUpperCase() + ': JOBSERVER and SERVERGROUP are null \n';
...
//for( i = 0 ;i <= process_name.length; i++ )
//{
//		query = 'select top 1 * from "_SYS_BIC"."cdp.ds.models/AT_HANA_DS_CON_INFO"';
//		 pcall = conn.prepareStatement(query);
//		 rs = pcall.executeQuery();
...
//		 pcall_job = conn.prepareStatement(query);
//		 rs_job = pcall_job.executeQuery();
//		if(rs_job.next())
//		{
//			job_name = rs_job.getNString(2);
