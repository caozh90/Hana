/************************************************************************
*name       :      
*function   :call ds for 6 reports
*input      ：
*output     :
*author     :shangqian
...
var montha =  String($.request.parameters.get("montha"));
var monthb =  String($.request.parameters.get("monthb"));
/********************************
* added by Chris Gao 2015-09-11
* to process rpt stdcost daily and monthly
*********************************/
if($.request.parameters.get("item") !== undefined)
{
	items = String($.request.parameters.get("item"));
}
// add new items (0000090Y6719,00D8213,00JX858,40K2500)
...
if(delta_cycle !== '' && delta_cycle !== undefined )
{
...
if(process_name === 'PRC_RPT_KEYSBB_ERROR')
{
	parameters += '\n'+'<variable name="G_IN_PRODUCTGROUP">'+cycle+'</variable>';
}
if(process_name === 'PRC_RPT_MTM')// add if by zhaodan1 20161010
{	
	parameters += '\n'+'<variable name="G_V_CYCLE_MONTH">'+montha+'</variable>';
	parameters += '\n'+'<variable name="G_V_DELTACYCLE_MONTH">'+monthb+'</variable>';
}
/********************************
* added by Chris Gao 2015-09-11
* to process rpt stdcost daily and monthly
*********************************/
if(process_name === 'PRC_RPT_STDCOST_DAILY' || process_name === 'PRC_RPT_STDCOST_MONTHLY')
{
