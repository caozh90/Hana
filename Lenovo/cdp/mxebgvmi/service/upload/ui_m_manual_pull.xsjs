...
var role1 = 'manualpull::uploadable';
var role2 = 'pullfromsupplier::uploadable';
var role3 = 'manualpackaging::uploadable';
...
var PRODUCTION_LINE;
var HEAD_SUPPLIERID;
var DELIVERY_DATE,temp_DELIVERY_DATE;//temp用于字符串转换成日期格式
var MATNR;
var PULL_QTY;
...
var LINE_SUPPLIERID;
var REMARK;
var PULL_TYPE = 'MANUAL'; //在Manual Pull页面，默认为'MANUAL'
...
var pcall_seq;
var seq;//upload seq
var MANUALPULLTYPE;//ZHAODAN1 20161108

function checkManualPullType(manualPullType){
	result = 'NO';
...
function exception_process()
{
	...
	var info_sql = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_m_manual_pull_err"('
		+'?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
	...
	try
	{
		...
		pcall2.setString(21,MANUALPULLTYPE);//ZHAODAN1
...
//验证权限
if(lib.assertAppRole(role1)==='OK'||lib.assertAppRole(role2)==='OK'||lib.assertAppRole(role3)==='OK')
{
	if(data.length < 1)
	{
		$.response.status = $.net.http.BAD_REQUEST;
	...
	for(i=0;i<data.length;i++)
	{
		...
		MANUALPULLTYPE = processData(data[i].MANUAL_PULL_TYPE);//zhaodan1 20161108
	...
	/*	if(DEST_STORAGE_LOC === undefined ||DEST_STORAGE_TYPE === undefined ||DEST_STORAGE_BIN === undefined ||SRC_WERKS === undefined )
		...
		*/
		if(checkManualPullType(MANUALPULLTYPE)==='NO'){
			error_message='ManualPullType not exist';
			position = i+1;
			exception_process();
			continue;			
		}
		...
		query = 'CALL "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_m_manual_pull"('
				+'?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
		try
		{
			...
			pcall.setString(21,MANUALPULLTYPE);//ZHAODAN1
			pcall.execute();
			pcall.close();
			conn.commit();
			body = 'Finished! You can check the result on HANA';
