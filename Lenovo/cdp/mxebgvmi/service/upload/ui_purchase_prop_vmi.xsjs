...
if(lib.assertAppRole(role)==='OK')
{
	...
	for(i=0;i<data.length;i++)
	{
		...
		query = 'CALL "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_purchase_prop_vmi"'
		    + '(\''
			+ITEM +'\',\''
			+VENDORID +'\',\''
			+LOGICAL_PLANT +'\',\''
	...
	//query = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.ui_purchase_proportion::validationProportion"'
	query = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.ui_purchase_proportion::validationProportion2"'
	    + '(\''
		+ seq +'\',\''
		+ fileName+'\''
		+')';
