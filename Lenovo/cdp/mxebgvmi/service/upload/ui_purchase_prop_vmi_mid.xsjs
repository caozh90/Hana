...
/*	var info_sql = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_purchase_prop_vmi_err"(\''
		+error_message+'\',\''
		+fileName+'\',\''
	...
	try
	{
		...
		pcall2.setDecimal(6,PROPORTION);
...
if(lib.assertAppRole(role)==='OK')
{
	 ...
	 * 清空buffer表的数据
	 */
	query = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.ui_purchase_proportion::clearBuffer"(?)';
	var pcall_clear = conn.prepareCall(query);
	pcall_clear.setInteger(1,seq);
	pcall_clear.execute();
	conn.commit();
	pcall_clear.close();
...
/*		query = 'CALL "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_purchase_prop_vmi_raw"'
		    + '(\''
			+ITEM +'\',\''
			+VENDORID +'\',\''
			+LOGICAL_PLANT +'\',\''
...
/*	query = 'call "MXEBGVMI"."cdp.mxebgvmi.procedures.pkg_ui.upload::ui_purchase_prop_vmi_validate"'
	    + '(\''
		+ seq +'\',\''
		+ fileName+'\''
		+')';*/
	...
	try
	{
		pcall_validate.setInteger(1,seq);
