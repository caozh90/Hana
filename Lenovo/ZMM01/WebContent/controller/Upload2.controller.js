sap.ui.define([
	...
	return BaseController.extend("lenovo.mtm.controller.Upload2", {
		...
		onDownloadTemplate: function(){
			...
			var url = "http://" + oHost + "/sap/bc/ui5_ui5/sap/zmm_mtm_001/templates/ODMQuotation.csv"
			window.location = url;
		}, 
			...
			reader.onload = function(e) {
			    ...
			    for (var i=0; i<allTextLines.length; i++) {
			    	...
			    	if(oCount == 1){
			    		...
			    		tarr[2] = tarr[2].replace(",","");
			    	}			        
			    	for (var j=0; j<tarr.length; j++) {
			    		Arr.push(tarr[j]);
			    	}
			    	that.pushResult.push(Arr);
			    }	
//			    that.handleUploadComplete(Event);
			};
...
//			}
			...
			for(var i = 1; i< this.pushResult.length-1; i++){
				...
				child = {
					...
					MfgPrice: this.pushResult[i][1],
					Zflag: this.pushResult[i][2],
				};
				if (child.Zflag != "Y" && child.Zflag != "N" ){
					MessageToast.show("Pls input 'Y' or 'N' for Comments flag");
					this.onCloseDialog();
					return;
				}
			...
			for(var i=0; i<oTable.getMaxItemsCount(); i++){
				if(oTable.getItems()[i] != undefined){
					if ((oTable.getItems()[i].getCells()[0].getSelected()) == true){
						...
						var oZmfgpc = oTable.getItems()[i].getCells()[3].getText();
						var oZflag = oTable.getItems()[i].getCells()[4].getText().substring(0,1);
						var oEntry = {Matnr:oMatnr,Zmfgpc:oZmfgpc,Zflag:oZflag,Zmode:"QUOTATION"};
						var batchChanges = [];
