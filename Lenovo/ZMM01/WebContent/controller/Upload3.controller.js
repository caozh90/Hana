sap.ui.define([
	...
	return BaseController.extend("lenovo.mtm.controller.Upload3", {
		...
		onDownloadTemplate: function(){
			...
			var url = "http://" + oHost + "/sap/bc/ui5_ui5/sap/zmm_mtm_001/templates/Quodate.csv"
			window.location = url;
		}, 
		...
		onUpload: function(oEvent){
			...
			reader.onload = function(e) {
			    ...
			    for (var i=0; i<allTextLines.length; i++) {
			    	...
			    	var oCount = data[0].split('"').length;
			    	if(oCount > 3){
			    	    that.oError = "X";
						MessageToast.show("The Double quotation marks exists in Template,Pls check it !!!");
						that.onCloseDialog();
						return;
					}
			    	if(oCount == 1){
			    		...
			    		tarr = data[0].split('"');
			    		//拆分comments前的6列数据
			    		Brr[0] = tarr[0].split(',')[0];
			    		Brr[1] = tarr[0].split(',')[1];
			    		Brr[2] = tarr[0].split(',')[2];
			    		Brr[3] = tarr[0].split(',')[3];
			    		Brr[4] = tarr[0].split(',')[4];
			    		Brr[5] = tarr[0].split(',')[5];
			    		Brr[6] = tarr[0].split(',')[6];
			    		Brr[7] = tarr[1];
			    		//拆分comments后的5列数据
			    		Brr[8] = tarr[2].split(',')[1];
			    		Brr[9] = tarr[2].split(',')[2];
			    		Brr[10] = tarr[2].split(',')[3];
			    		Brr[11] = tarr[2].split(',')[4];
			    		Brr[12] = tarr[2].split(',')[5];
			    	}			        
			    	for (var j=0; j<Brr.length; j++) {
			    		Crr.push(Brr[j]);
			    	}
			    	that.pushResult.push(Crr);
			    }			    
			};
//			reader.readAsBinaryString(file);
		...
		handleUploadComplete: function(oEvent){
		    ...
		    //保持model数据为空
		    var EmptyData = {data:[]};
		    this.oUploadModel.setData(EmptyData);
...
//			}
			...
			for(var i = 1; i< this.pushResult.length-1; i++){
				...
				child = {
					...
					Quodate: this.pushResult[i][1],
					Zeowdate: this.pushResult[i][2],
					Zad: this.pushResult[i][3],
					Zvendor: this.pushResult[i][4],
				    Lifnr: this.pushResult[i][5],
				    Zohf: this.pushResult[i][6],
				    Zlwc: this.pushResult[i][7],
				    Zio: this.pushResult[i][8],
				    Zbomlt: this.pushResult[i][9],
				    Zqutolt: this.pushResult[i][10],
				    Zcontlt: this.pushResult[i][11],
				    Zoverlt: this.pushResult[i][12],
				};
				var filter = [];
			...
			for(var i=0; i<oTable.getMaxItemsCount(); i++){
				if(oTable.getItems()[i] != undefined){
					if ((oTable.getItems()[i].getCells()[0].getSelected()) == true){
						...
						var oZqd = oTable.getItems()[i].getCells()[3].getText().substring(0,8);
						var Zeowdate = oTable.getItems()[i].getCells()[4].getText().substring(0,8);
						var Zad = oTable.getItems()[i].getCells()[5].getText().substring(0,8);
						var Zvendor = oTable.getItems()[i].getCells()[6].getText().substring(0,12);
						var Lifnr = oTable.getItems()[i].getCells()[7].getText().substring(0,10);
						var Zohf = oTable.getItems()[i].getCells()[8].getText().substring(0,10);
						var Zlwc = oTable.getItems()[i].getCells()[9].getText();
						var Zio = oTable.getItems()[i].getCells()[10].getText().substring(0,20);
						var Zbomlt = oTable.getItems()[i].getCells()[11].getText().substring(0,10);
						var Zqutolt = oTable.getItems()[i].getCells()[12].getText().substring(0,10);
						var Zcontlt = oTable.getItems()[i].getCells()[13].getText().substring(0,10);
						var Zoverlt = oTable.getItems()[i].getCells()[14].getText().substring(0,10);
						var oEntry = {Matnr:oMatnr,Zqd:oZqd,Zeowdate:Zeowdate,Zad:Zad,Zvendor:Zvendor,Lifnr:Lifnr,Zohf:Zohf,
								Zlwc:Zlwc,Zio:Zio,Zbomlt:Zbomlt,Zqutolt:Zqutolt,Zcontlt:Zcontlt,Zoverlt:Zoverlt,Zmode:"QUODATE"};
						var batchChanges = [];
