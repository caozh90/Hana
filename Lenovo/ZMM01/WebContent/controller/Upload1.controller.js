sap.ui.define([
	...
	return BaseController.extend("lenovo.mtm.controller.Upload1", {
		...
		onDownloadTemplate: function(){
			...
			var url = "http://" + oHost + "/sap/bc/ui5_ui5/sap/zmm_mtm_001/templates/ODMBOM.csv"
			window.location = url;
		}, 
			...
			for(var i=0; i<oTable.getMaxItemsCount(); i++){
				if(oTable.getItems()[i] != undefined){
					if ((oTable.getItems()[i].getCells()[0].getSelected()) == true){
						...
						var oZmfgbc = oTable.getItems()[i].getCells()[3].getText();
						var oZflag = oTable.getItems()[i].getCells()[4].getText().substring(0,1);
						var oEntry = {Matnr:oMatnr,Zmfgbc:oZmfgbc,Zflag:oZflag,Zmode:"ODMBOM"};
						var batchChanges = [];
